...
jQuery.facebooklist = function(elem, list, complete, feed, ajax, height, filter,
		newel, def) {
	var annoninput;
	...
	var addItem = function(item, preadded) {
		var title = item.text();
		var value = (item.attr('rel') && item.attr('rel') != -1 ? item
				.attr('rel') : title);
		...
		if (!preadded) {
			holder.removeChild(/*document.getElementById('annoninput')*/annoninput);
			addInput();
		}
		feed.hide();
	}
		...
		if (filter.userfilter) {
			...
			$.each(feed.children('li:not([fckb])'), function(i, val) {
				...
				if (filter.casesensetive) {
					...
					item.html(item.text().replace(input,
							'<em>' + input + '</em>'));
				} else {
					flag = item.text().toLowerCase().indexOf(
							input.toLowerCase());
					item.html(item.text().replace(input,
							'<em>' + input + '</em>'));
				}
	...
	var bindEvents = function() {
		...
		maininput
				.keydown(function(event) {
					...
					if (event.keyCode == 40) {
						removeFeedEvent();
						if (typeof (nowFocusOn) == 'undefined'
								|| nowFocusOn.length == 0) {
							...
							var prev = parseInt(nowFocusOn
									.prevAll('li:visible').length, 10);
							var next = parseInt(nowFocusOn
									.nextAll('li:visible').length, 10);
							if ((prev > Math.round(height / 2) || next <= Math
									.round(height / 2))
									&& typeof (nowFocusOn.get(0)) != 'undefined') {
								feed.get(0).scrollTop = parseInt(nowFocusOn
										.get(0).scrollHeight, 10)
										* (prev - Math.round(height / 2));
							}
						}
						feed.children('li').removeClass("auto-focus");
					...
					if (event.keyCode == 38) {
						...
						if (typeof (nowFocusOn) == 'undefined'
							...
							feed.get(0).scrollTop = parseInt(
									nowFocusOn.get(0).scrollHeight, 10)
									* (parseInt(
											feed.children('li:visible').length,
											10) - Math.round(height / 2));
						} else {
							nowFocusOn.removeClass("auto-focus");
							...
							var next = parseInt(nowFocusOn
									.nextAll('li:visible').length, 10);
							if ((next > Math.round(height / 2) || prev <= Math
									.round(height / 2))
									&& typeof (nowFocusOn.get(0)) != 'undefined') {
								feed.get(0).scrollTop = parseInt(nowFocusOn
										.get(0).scrollHeight, 10)
	...
	var addInput = function() {
		//var li = document.createElement('li');
		annoninput = document.createElement('li');
		var li = annoninput;
		var input = document.createElement('input');
		$(li).attr({
			'class' : 'bit-input'/*,
					...
					if (event.keyCode != 40 && event.keyCode != 38) {
						...
						if(ajax.data) {
							...
							if (ajax.cache && cache.length > 0) {
								...
								$.getJSON(ajax.url + '?tag=' + etext, null,
										function(data) {
	...
	$(document).click(
			function(event) {
				if ($(event.target).attr('class') != 'holder'
						&& $(event.target).attr('class') != 'maininput') {
					//$('.default').hide();
					def.hide();
					$(feed).hide();
				}
