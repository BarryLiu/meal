...
$.extend(J, {
    ...
    toObject : function(json) { return eval("("+json+")"); },
    focus : function(jText) {
    	jText.focus();
    	var rng = jText[0].createTextRange();
    	rng.collapse(false);
    	rng.select();
    },
    optVals: function(jOptions) {
    	var vals = [];
    	var size = jOptions.size();
    	for(var i=0;i<size;i++) {
    		vals[i] = jOptions[i].value;
    	}
    	return vals; //$.makeArray(vals);
    },
    ...
    handleAjaxSuccess: function(data, callback) {
    	if(data.goPage) {
    		if(window.opener==null){window.top.location.href=data.url;}
    		else {window.opener.top.location.href==data.url;}
    	} else {
    		callback();
    	}
    },
