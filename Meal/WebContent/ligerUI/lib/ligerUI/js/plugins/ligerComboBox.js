    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (this.element.tagName.toLowerCase() == "input")
            {
                ...
                if(!p.isMultiSelect){
                    g.inputText.keydown(function() {
                        var code = event.keyCode;
                        var str = String.fromCharCode(code);
                        var reg=new RegExp("^"+str);
                        var reg1=new RegExp("^"+str.toLowerCase());
                        var num=0;
                        var flag=0;
                        for(var i=0;i<g.data.length;i++){
                            if(reg.test(toPY(g.data[i].text))||reg1.test(toPY(g.data[i].text))){
                                if(!p.showText) {
                                    if (g.getValue() == g.data[i].id) {
                                        num = 1;
                                        continue;
                                    }
                                    if (num > 0) {
                                        g.setValue(g.data[i].id);
                                        flag = 1;
                                        break;
                                    }
                                }else{
                                    if (g.getValue() == g.data[i].text) {
                                        num = 1;
                                        continue;
                                    }
                                    if (num > 0) {
                                        g.setValue(g.data[i].text);
                                        flag = 1;
                                        break;
                                    }
                                }
