...
(function ($)
{
    ...
    $.ligerDefaults.Form.editors.textarea =
    {
        create: function (container, editParm, p)
        {
            ...
            //这里的p是整个form控件的p，应该属于bug。这里应该判断当前field的p，而不是整个控件
            if (editParm.field.options && editParm.field.options.readonly) editor.attr("readonly", true);
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _createEditors : function(e)
        {
            ...
            {
                var container = $("#" + idPrev + "\\|" + fieldIndex, jform),
        ...
        initValidate: function ()
        {
            ...
            var validateOptions = $.extend({
                errorPlacement: function (lable, element)
                {
                    ...
                    //原语句没有处理label, 而在下文提交时, label的for属性必须指向该id. 
                    //所以这里在创建id时, 必须同时设置label. 
                    if (!element.attr("id")) {
